...
class SwissQRBill {
  ...
  _render() {
    ...
    if (this.data.amount !== void 0) {
      amountContainer.addTSpan(formatAmount(this.data.amount, this.data.amountLocale)).x(`${amountXPosition}mm`).fontFamily(this.font).fontWeight("normal").fontSize("8pt");
    } else {
    ...
    if (this.data.amount !== void 0) {
      paymentPartMiddleTextContainer.addTSpan(formatAmount(this.data.amount, this.data.amountLocale)).x("22mm").fontFamily(this.font).fontWeight("normal").fontSize("10pt");
    } else {
  ...
  _renderQRCode() {
    const qrCode = new SwissQRCode(this.data, void 0, this.qrTheme);
