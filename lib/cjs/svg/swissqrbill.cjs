...
class SwissQRBill {
  constructor(data, options) {
    ...
    this.qrTheme = options == null ? void 0 : options.qrTheme;
  ...
  _render() {
    ...
    if (this.data.amount !== void 0) {
      amountContainer.addTSpan(utils.formatAmount(this.data.amount, this.data.amountLocale)).x(`${amountXPosition}mm`).fontFamily(this.font).fontWeight("normal").fontSize("8pt");
    } else {
    ...
    if (this.data.amount !== void 0) {
      paymentPartMiddleTextContainer.addTSpan(utils.formatAmount(this.data.amount, this.data.amountLocale)).x("22mm").fontFamily(this.font).fontWeight("normal").fontSize("10pt");
    } else {
  ...
  _renderQRCode() {
    const qrCode = new swissqrcode.SwissQRCode(this.data, void 0, this.qrTheme);
